...
export default class CaseDocumentPdfMerger extends LightningElement {
    ...
    async mergePDFs(base64List, sortedDocuments) {
        try {
            ...
            if (pagesAdded === 0) {
                console.error('No valid PDF pages were processed. DHS_All documents may be corrupted or invalid.');
                if (window.$) {
                    $('#loadingOverlay div[style*="font-size: 18px"]').text('No Valid Documents Found');
                    $('#loadingOverlay div[style*="font-size: 14px"]').text('DHS_All selected documents appear to be corrupted or not valid PDF files.');
                    $('#loadingOverlay .spinner-border').hide();
