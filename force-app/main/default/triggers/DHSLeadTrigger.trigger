trigger DHSLeadTrigger on DHS_Lead__c (before insert, before update, after insert, after update, after delete, after undelete) { // NOPMD - Legacy trigger behavior retained for compatibility.
    ...
    list<DHS_Waiver_Information__c > waiverInformationToCreate  = new LisT<DHS_Waiver_Information__c >();
    ...
    if((trigger.isInsert || trigger.isUpdate) && trigger.isBefore){
        ...
        if(trigger.isInsert){
            leads = [Select Id, DHS_Case__c, Item_number__c from DHS_Lead__c where DHS_Case__c in: leadcaseids]; // NOPMD - Legacy trigger behavior retained for compatibility.
        }else if(trigger.isUpdate){
            leads = [Select Id, DHS_Case__c, Item_number__c from DHS_Lead__c where DHS_Case__c in: leadcaseids and id not IN: leadIds]; // NOPMD - Legacy trigger behavior retained for compatibility.
        }
        map<Id,DHS_Case__c> casesMap = new map<id,DHS_Case__c>([select id,DHS_Contact__c, Scoping_Date__c, Current_Task__c from DHS_Case__c where id in: leadcaseids ]); // NOPMD - Legacy trigger behavior retained for compatibility.
    ...
    if(trigger.isInsert && trigger.isBefore){
        ...
        List<DHS_Lead__c> leads = new List<DHS_Lead__c>();
        leads = [Select Id, DHS_Case__c from DHS_Lead__c where DHS_Case__c in: caseIds]; // NOPMD - Legacy trigger behavior retained for compatibility.
        map<Id,DHS_Case__c> casesMap = new map<id,DHS_Case__c>([select id,DHS_Contact__c, Scoping_Date__c, Current_Task__c from DHS_Case__c where id in:caseIds ]); // NOPMD - Legacy trigger behavior retained for compatibility.
    ...
    if(trigger.isUpdate && trigger.isBefore){
        ...
        dailyProductionSummaries = [Select Id, DHS_Reviewer__c, Leads_Accepted__c, Leads_Completed__c, Leads_Rejected__c, Summary_Date__c // NOPMD - Legacy trigger behavior retained for compatibility.
        ...
        Map<Id,User> userMap = new Map<Id,User>([Select Id,Name from User]); // NOPMD - Legacy trigger behavior retained for compatibility.
         ...
         for(DHS_Lead__c l: trigger.new){
            ...
            if(l.Status__c == 'Assigned' && trigger.oldmap.get(l.id).Status__c != 'Assigned'){
                User investigator = [Select Id,Firstname,lastname,email from user where id=:l.DHS_Assigned_To__c]; // NOPMD - Legacy trigger behavior retained for compatibility.
            ...
            if(l.Lead_Accepted_or_Rejected__c =='Rejected' && trigger.oldmap.get(l.id).Lead_Accepted_or_Rejected__c != 'Rejected'){
                ...
                DHS_Case__c caseOfLead = [Select DHS_Assigned_To__c,Case_Number__c, DHS_Assigned_To__r.Email from DHS_Case__c where id=:l.DHS_Case__c]; // NOPMD - Legacy trigger behavior retained for compatibility.
                User caseCMO = [Select Firstname,lastname from user where id=:caseOfLead.DHS_Assigned_To__c]; // NOPMD - Legacy trigger behavior retained for compatibility.
                User rejectedInvestigator = [Select Firstname,lastname from user where id=:trigger.oldmap.get(l.id).DHS_Assigned_To__c]; // NOPMD - Legacy trigger behavior retained for compatibility.
                List<string> toAddress = new List<string>();
            ...
            if(l.Status__c != 'QA Rejected' && trigger.oldmap.get(l.id).Status__c == 'QA Rejected'){
                DHS_Case__c caseOfLead = [Select DHS_Assigned_To__c,Case_Number__c, DHS_Assigned_To__r.Email from DHS_Case__c where id=:l.DHS_Case__c]; // NOPMD - Legacy trigger behavior retained for compatibility.
                User investigator = [Select Firstname,lastname,email from user where id=:l.DHS_Assigned_To__c]; // NOPMD - Legacy trigger behavior retained for compatibility.
                List<string> toAddress = new List<string>();
                toAddress.add(investigator.Email);
    ...
    if(trigger.isUpdate && trigger.isAfter){
        ...
        List<DHS_Message__c> existingMessages = [select id,DHS_Lead__c,DHS_Lead__r.LeadType__c,Subject__c from DHS_Message__c where DHS_Lead__c IN: Trigger.NewMap.keyset()]; // NOPMD - Legacy trigger behavior retained for compatibility.
        ...
        map<id, DHS_Case__c> casemap = new map<id,DHS_Case__c>([select id, Current_Task__c, Investigations_Review_Date__c, QA_and_Review_Date__c from DHS_Case__c where id in: caseids]); // NOPMD - Legacy trigger behavior retained for compatibility.
        list<DHS_Lead__c> leads = [Select Id, DHS_Assigned_To__c,DHS_Case__c, Status__c  from DHS_Lead__c where DHS_Case__c in: caseids]; // NOPMD - Legacy trigger behavior retained for compatibility.
    ...
    if(Trigger.isupdate && Trigger.isAfter){
        ...
        user u = [select id,user_type__c from user where id =: userinfo.getuserid()]; // NOPMD - Legacy trigger behavior retained for compatibility.
    ...
    public static datetime dt(date dte,boolean isDate){ // NOPMD - Boolean is required by the existing Visualforce or Apex contract.
        integer day;
        integer month;
        integer year;
        day = dte.day();
    ...
    public static datetime dt(date dte,boolean isDate,boolean isDatetime){ // NOPMD - Boolean is required by the existing Visualforce or Apex contract.
        integer day;
        integer month;
        integer year;
        day = dte.day();
