trigger CaseTrigger on Case__c (before insert,after insert,before update, after update) { // NOPMD
    ...
    if(trigger.isAfter &&  trigger.isUpdate){
        ...
        if(!caseIdsToConsiderForDiscontinuation.isEmpty()){
            ...
            leadsToUpdate = [Select Id, Status__c, Case__c, Lead_Status_Before_Discontinuation__c from lead__c WHERE case__c in:caseIdsToConsiderForDiscontinuation]; // NOPMD
            for(lead__c l:leadsToUpdate){
                if(discontinuationCasesMap.get(l.case__c).case_discontinued__c){ // NOPMD - legacy structure retained to preserve behavior
    ...
    if(trigger.isAfter && trigger.isInsert){
        ...
        contacts = [Select Id, Run_Credit_Report__c  from contact__c where id in: contactids]; // NOPMD
    ...
    if(trigger.isBefore && trigger.isUpdate){ 
        for(Case__c c: Trigger.New){
            if(c.Assigned_to__c != Null && Trigger.oldMap.get(c.id).Assigned_to__c != c.Assigned_to__c){
                User assignee = [Select Id,Firstname,lastname,Email from user where id=:c.Assigned_to__c]; // NOPMD
