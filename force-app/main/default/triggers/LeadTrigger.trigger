trigger LeadTrigger on Lead__c (before insert, before update, after insert, after update) { // NOPMD
    ...
    list<Waiver_Information__c > WaiverInformationToCreate  = new LisT<Waiver_Information__c >(); // NOPMD
    ...
    if((trigger.isInsert || trigger.isUpdate) && trigger.isBefore){
        ...
        if(trigger.isInsert){
            leads = [Select Id, Case__c, item_Number__c from Lead__C where case__c in: leadcaseids]; // NOPMD
        }else if(trigger.isUpdate){
            leads = [Select Id, Case__c, item_Number__c from Lead__C where case__c in: leadcaseids and id not IN: leadIds]; // NOPMD
        }
        map<Id,case__c> casesMap = new map<id,case__c>([select id,contact__c, Scoping_Date__c, current_task__c from case__c where id in: leadcaseids ]); // NOPMD
    ...
    if(trigger.isInsert && trigger.isBefore){
        ...
        List<Lead__c> leads = new List<Lead__c>();
        leads = [Select Id, Case__c from Lead__C where case__c in: caseIds]; // NOPMD
        map<Id,case__c> casesMap = new map<id,case__c>([select id,contact__c, Scoping_Date__c, current_task__c from case__c where id in:caseIds ]); // NOPMD
        ...
        for(Lead__c l: Trigger.New){
            ...
            if(caseIdNnumberOfleads.containsKey(l.case__c)){ // NOPMD
    ...
    if(trigger.isUpdate && trigger.isBefore){
        ...
        dailyProductionSummaries = [Select Id, Reviewer__c, Leads_Accepted__c, Leads_Completed__c, Leads_Rejected__c, Summary_Date__c // NOPMD
        ...
        Id orgWideEmailId = MailUtilities.getNoReplyOrgWideEmailId();
         for(lead__c l: trigger.new){
            ...
            if(l.status__c == 'Assigned Not Accepted' && trigger.oldmap.get(l.id).status__c != 'Assigned Not Accepted'){
                User investigator = [Select Id,Firstname,lastname,email,email__c from user where id=:l.Assigned_to__c]; // NOPMD
                ...
                if (orgWideEmailId != null) { // NOPMD - legacy structure retained to preserve behavior
            ...
            if(l.Lead_Accepted_or_Rejected__c =='Rejected' && trigger.oldmap.get(l.id).Lead_Accepted_or_Rejected__c != 'Rejected'){
                ...
                Case__c caseOfLead = [Select Assigned_to__r.Email__c,Assigned_To__c,Case_Number__c, Assigned_To__r.Email from case__c where id=:l.case__c]; // NOPMD
                User caseCMO = [Select Firstname,lastname from user where id=:caseOfLead.Assigned_to__c]; // NOPMD
                User rejectedInvestigator = [Select Firstname,lastname from user where id=:trigger.oldmap.get(l.id).Assigned_to__c]; // NOPMD
                List<string> toAddress = new List<string>();
                ...
                Body = Body + 'Thanks<br/>ISN Corporation';
                system.debug('Rejection is doing emails'); // NOPMD
                Messaging.SingleEmailMessage mail = new Messaging.SingleEmailMessage();             
                mail.setSaveAsActivity(false);
                mail.setSubject(subject);
                mail.setBccSender(false);
                ...
                if(String.isBlank(l.Lead_Log__c)){ // NOPMD - legacy structure retained to preserve behavior
                    ...
                    System.debug('l.assigned_to__c...'+l.assigned_to__c); // NOPMD
            ...
            if(l.status__c == 'Completed' && trigger.oldmap.get(l.id).status__c != 'Completed'){
                if(investigatorSummaryMap.get(l.assigned_to__c) != null){ // NOPMD - legacy structure retained to preserve behavior
                    Daily_Production_Summary__c dps = investigatorSummaryMap.get(l.assigned_to__c);
            ...
            if(l.status__c == 'QA Rejected' && trigger.oldmap.get(l.id).status__c != 'QA Rejected'){
                Case__c caseOfLead = [Select Assigned_To__c,Case_Number__c, Assigned_To__r.Email from case__c where id=:l.case__c]; // NOPMD
                User investigator = [Select Firstname,lastname,email,Email__c from user where id=:l.Assigned_to__c]; // NOPMD
    ...
    if(trigger.isUpdate && trigger.isAfter){
        ...
        List<Message__c> existingMessages = [select id,Lead__c,Lead__r.leadType__c,subject__c from message__c where Lead__c IN: Trigger.NewMap.keyset()]; // NOPMD
        ...
        for(Lead__C l: Trigger.new){
            If(Trigger.newMap.get(l.Id).Status__c == 'Completed'){
                If (l.Issues_discrepancies_or_developed_acti__c== 'Yes'){  // NOPMD - legacy structure retained to preserve behavior
                    system.debug('l.Issues_discrepancies_or_developed_acti__c...'+l.Issues_discrepancies_or_developed_acti__c); // NOPMD
            ...
            If(Trigger.newMap.get(l.Id).Status__c == 'Completed' && l.leadtype__c == 'Subject Interview (ESI/TESI)'){
                if(leadNMessages.containsKey(l.Id)){ // NOPMD - legacy structure retained to preserve behavior
                    system.debug('Trigger.newMap.get(l.Id).Explanation_of_why_leads_not_obtained__c updated...'+l.Id+Trigger.newMap.get(l.Id).Explanation_of_why_leads_not_obtained__c); // NOPMD
        ...
        map<id, case__c> casemap = new map<id,case__c>([select id, current_task__c, Investigations_Review_Date__c, QA_and_review_Date__c from case__c where id in: caseids]); // NOPMD
        list<lead__c> leads = [Select Id, Assigned_to__c,case__c, Status__c  from Lead__c where case__c in: caseids]; // NOPMD
    ...
    if(Trigger.isupdate && Trigger.isAfter){
        ...
        if(u.user_type__c == 'INV' || Test.isRunningTest()){
            List<timesheet__c> invTimeSheet = [select id,Associated_Leads__c from timesheet__c where user__c =: userinfo.getuserid() and date__c =: system.today() limit 1]; // NOPMD
            
            List<Lead__c> leads = [select id,case__r.Case_number__c,leadtype__c,coverage_type__c,item_number__c from Lead__c where Id IN:Trigger.NewMap.keyset()]; // NOPMD
